"""Causal Transformer for autoregressive modeling of molecular coordinates.

This module implements a GPT-like transformer architecture for autoregressive
generation of discretized molecular coordinates, similar to PixelCNN but using
attention instead of convolutions.
"""
...
class CausalSelfAttention(nn.Module):
    """Causal (masked) self-attention layer with KV-cache and Flash Attention support.

    Only attends to previous positions in the sequence, ensuring
    autoregressive property. Supports KV-caching for fast inference.
    """
    ...
    def forward(
        ...
        if self.use_flash_attention and attention_mask is None:
            # Flash attention handles causal masking efficiently
            is_causal = past_kv is None  # Only use causal mask for non-cached forward
            ...
            # Apply causal mask
            # When using cache, we only need to mask the query positions
            if past_kv is not None:
                # For cached inference, q has seq_len=1, no causal mask needed within q
                # but we need to ensure we can attend to all past positions
...
class SwiGLU(nn.Module):
    ...
    Computes: SwiGLU(x, W, V, b, c) = Swish(xW + b) * (xV + c)
    """
...
class FeedForward(nn.Module):
    """Feed-forward network with GELU or SwiGLU activation."""
    ...
    def __init__(
        ...
        if use_swiglu:
            ...
            # since it has two projections instead of one
            swiglu_hidden = int(2 * hidden_dim / 3)
            self.net = nn.Sequential(
                SwiGLU(dim, swiglu_hidden),
                nn.Dropout(dropout),
                nn.Linear(swiglu_hidden, dim),
                nn.Dropout(dropout),
            )
        else:
            self.net = nn.Sequential(
...
class CrossAttention(nn.Module):
    """Cross-attention layer for attending to conditioning information.

    Unlike causal self-attention, this allows full attention to all conditioning tokens.
    Includes stability features: zero-init output projection and optional QK-norm.
    """
    ...
    def __init__(
        ...
        self.kv_proj = nn.Linear(dim, 2 * self.inner_dim, bias=False)
        self.out_proj = nn.Linear(self.inner_dim, dim, bias=False)
        self.dropout = nn.Dropout(dropout)

        # QK-norm for stability (normalizes queries and keys before attention)
        if use_qk_norm:
            self.q_norm = RMSNorm(self.head_dim)
            self.k_norm = RMSNorm(self.head_dim)

        # Zero-initialize output projection for stable training
        # This ensures cross-attention starts with zero contribution
        if zero_init_output:
    ...
    def forward(
        ...
        Args:
            x: Query tensor (coordinate tokens), shape (batch, seq_len, dim)
            context: Key/Value tensor (conditioning), shape (batch, context_len, dim)
            context_mask: Optional mask for context, shape (batch, context_len)
        ...
        kv = self.kv_proj(context).reshape(batch_size, context_len, 2, self.num_heads, self.head_dim)
        kv = kv.permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]

        # Apply QK-norm for stability (prevents large attention logits)
        if self.use_qk_norm:
            # Reshape for per-head normalization: (batch, heads, seq, head_dim)
            q = self.q_norm(q)
            k = self.k_norm(k)
...
class CausalTransformerBlock(nn.Module):
    """Single transformer block with causal attention, optional cross-attention,
    KV-cache, and Flash Attention support."""
    ...
    def __init__(
        ...
        cross_attn_zero_init: bool = False,
        ...
        if use_cross_attention:
            self.cross_attn = CrossAttention(
                ...
                dropout=dropout,
                use_flash_attention=use_flash_attention,
                use_qk_norm=cross_attn_qk_norm,
                zero_init_output=cross_attn_zero_init,
            )
            # Learnable gate for cross-attention output (initialized to 0 via tanh)
            # This allows the model to gradually learn to use cross-attention
            if cross_attn_gate:
                self.cross_attn_gate = nn.Parameter(torch.zeros(1))

        self.ff = FeedForward(dim=dim, expansion=expansion, dropout=dropout, use_swiglu=use_swiglu)
    ...
    def forward(
        ...
        # 1. Causal self-attention over coordinates
        attn_out, present_kv = self.attn(self.norm1(x), attention_mask, past_kv, use_cache)
        x = x + attn_out

        # 2. Cross-attention to conditioning (if enabled and context provided)
        if self.use_cross_attention and context is not None:
            cross_out = self.cross_attn(self.norm_cross(x), context, context_mask)
            # Apply learnable tanh gate (starts at 0, gradually opens during training)
            if self.use_cross_attn_gate:
...
class CausalTransformer(nn.Module):
    """Causal Transformer for autoregressive coordinate generation.

    This model predicts the next coordinate token given all previous tokens,
    outputting a distribution over discrete bins.

    Args:
        num_bins: Number of discrete bins for each coordinate
        input_dim: Dimension of each input position (typically 3 for 3D coordinates)
        channels: Hidden dimension of the transformer
        ...
        use_swiglu: Whether to use SwiGLU activation instead of GELU. Default: True
        ...
        use_flash_attention: Whether to use Flash Attention via PyTorch's
            scaled_dot_product_attention for memory efficiency. Default: True
        use_bf16: Whether to use bfloat16 precision for forward pass. Default: False
        use_cross_attention: Whether to use cross-attention for conditioning (unmasked)
            instead of adding conditioning embeddings (which are causally masked). Default: False
        cross_attn_qk_norm: Whether to use QK-norm in cross-attention for stability. Default: False
        cross_attn_zero_init: Whether to zero-initialize cross-attention output projection. Default: False
        cross_attn_gate: Whether to use learnable tanh gate for cross-attention. Default: False
        add_cond_to_input: Whether to add conditioning embeddings directly to input token
            embeddings (in addition to or instead of cross-attention). Default: False
        use_global_cond: Whether to use global (non-causal) conditioning. When True,
            each position attends to all conditioning embeddings before the transformer.
            Similar to cross-attention but applied once upfront. Default: False
        use_input_norm: Whether to apply layer normalization after combining all input
            embeddings (token, position, conditioning) before transformer blocks. Default: False
    """
    ...
    def __init__(
        ...
        self.use_cross_attention = use_cross_attention
        self.add_cond_to_input = add_cond_to_input
        self.use_global_cond = use_global_cond
        self.use_input_norm = use_input_norm

        # Input embedding for discrete tokens
        if coordinate_embedding_type == "embedding":
            self.token_embed = nn.Embedding(num_bins, channels)
        else:
            # Linear projection from one-hot or continuous
            self.token_embed = nn.Linear(num_bins, channels)

        self.coordinate_embedding_type = coordinate_embedding_type

        # Positional embeddings
        if pos_embed_type == "learned":
            self.pos_embed = nn.Parameter(torch.randn(1, self.max_seq_len, channels) * 0.02)
        else:
            self.pos_embed = SinusoidalPositionalEmbedding(channels, self.max_seq_len)
        ...
        # Global conditioning attention (h attends to all conditioning tokens)
        if use_global_cond:
            self.global_cond_attn = nn.MultiheadAttention(
                embed_dim=channels,
                num_heads=num_heads,
                dropout=dropout,
                batch_first=True,
        ...
        self.blocks = nn.ModuleList(
            [
                CausalTransformerBlock(
                    ...
                    cross_attn_zero_init=cross_attn_zero_init,
        ...
        # Input norm (after combining all embeddings, before transformer blocks)
        if use_input_norm:
            if use_rms_norm:
        ...
        self.output_proj = nn.Linear(channels, num_bins, bias=False)
    ...
    def forward(
        ...
        Args:
            x: Discrete token indices, shape (batch, seq_len) where seq_len = num_atoms * input_dim
    ...
    def _forward_impl(
        ...
        if self.coordinate_embedding_type == "embedding":
            h = self.token_embed(x)  # (batch, seq_len, channels)
        else:
            # One-hot encoding
            x_onehot = F.one_hot(x, num_classes=self.num_bins).float()
            h = self.token_embed(x_onehot)

        # Add positional embeddings
        positions = torch.arange(position_offset, position_offset + seq_len, device=device)
        if self.pos_embed_type == "learned":
            h = h + self.pos_embed[:, position_offset : position_offset + seq_len, :]
        else:
            # For sinusoidal, we need to handle the offset
            h = h + self.pos_embed.pe[position_offset : position_offset + seq_len]
        ...
        if self.cond_embed is not None and encodings is not None:
            ...
            if self.add_cond_to_input or (not self.use_cross_attention and not self.use_global_cond):
                # Add conditioning embeddings directly to input (causally masked)
                # Repeat for each coordinate dimension
                cond_expanded = cond.repeat_interleave(self.input_dim, dim=1)  # (batch, total_seq_len, channels)
                h = h + cond_expanded[:, position_offset : position_offset + seq_len, :]

            if self.use_global_cond:
                # Global conditioning: each position in h attends to all conditioning tokens
                # Create key padding mask if node_mask provided (True = ignore)
                key_padding_mask = ~node_mask if node_mask is not None else None
                global_cond, _ = self.global_cond_attn(
                    h, cond, cond, key_padding_mask=key_padding_mask
                )  # (batch, seq_len, channels)
                h = h + global_cond

            elif self.use_cross_attention:
                # Use cross-attention: conditioning is attended to without causal mask
                # This allows all coordinate tokens to see full molecule info
                context = cond  # (batch, num_atoms, channels)
                context_mask = node_mask  # (batch, num_atoms) or None

        # Normalize combined embeddings before transformer
        if self.use_input_norm:
            h = self.input_norm(h)

        # Create attention mask from node_mask if provided (for self-attention)
        attention_mask = None
        if node_mask is not None and not self.use_cross_attention:
            # Only use attention mask in self-attention if not using cross-attention
            # (when using cross-attention, node_mask is used for context_mask instead)
            full_attention_mask = node_mask.repeat_interleave(self.input_dim, dim=1)  # (batch, total_seq_len)
        ...
        for i, block in enumerate(self.blocks):
            ...
            h, present_kv = block(h, attention_mask, past_kv, use_cache, context=context, context_mask=context_mask)
            if use_cache:
    ...
    def generate(
        ...
        Args:
            ...
            node_mask: Optional mask for padding atoms, shape (batch, num_atoms)
        ...
        # Pre-compute context embeddings for cross-attention (if using cross-attention)
        # This is computed once and reused for all generation steps
        context = None
        context_mask = None
        if self.use_cross_attention and self.cond_embed is not None and encodings is not None:
            context = self.cond_embed(encodings)  # (batch, num_atoms, channels)
            context_mask = node_mask  # (batch, num_atoms) or None
        ...
        for i in range(seq_len):
            ...
            if use_cache and past_kvs is not None:
                ...
                logits, past_kvs = self._generate_step(
                    generated[:, -1:],
                    encodings,
                    node_mask,
                    past_kvs=past_kvs,
                ...
                logits = logits[:, -1, :]  # (batch, num_bins)
            else:
                # First token or no cache - process full sequence
                logits, past_kvs = self._generate_step(
                    generated,
                    encodings,
                    node_mask,
    ...
    def _generate_step(
        ...
        """Single generation step with pre-computed context for cross-attention.

        This avoids recomputing conditional embeddings at each step.
        """
        batch_size, seq_len = x.shape
        device = x.device
        ...
        # Add coordinate dimension embeddings
        coord_dims = positions % self.input_dim
        h = h + self.coord_dim_embed(coord_dims)
        ...
        if self.cond_embed is not None and encodings is not None:
            ...
            if self.add_cond_to_input or (not self.use_cross_attention and not self.use_global_cond):
                # Add conditioning embeddings directly to input (causally masked)
                cond_expanded = cond.repeat_interleave(self.input_dim, dim=1)
                h = h + cond_expanded[:, position_offset : position_offset + seq_len, :]
        ...
        # Attention mask for self-attention (only if not using cross-attention)
        attention_mask = None
        if node_mask is not None and not self.use_cross_attention:
...
class ConditionalEmbedder(nn.Module):
    ...
    Embeds atom type, amino acid type, amino acid position, atom position (index),
    and sequence length to condition the autoregressive model on the molecular structure.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            use_atom_pos_embed: Whether to include atom position (index) embedding
        """
        super().__init__()

        self.use_atom_pos_embed = use_atom_pos_embed
        ...
        if use_atom_pos_embed:
            self.atom_pos_embed = SinusoidalEmbedding(embed_size=hidden_dim, div_value=sinusoid_div_value)
            proj_input_dim = 5 * hidden_dim
        else:
            proj_input_dim = 4 * hidden_dim
    ...
    def forward(self, encodings: dict) -> torch.Tensor:
        ...
        batch_size, num_atoms = atom_type.shape
        ...
        aa_pos_emb = self.residue_pos_embed(aa_pos)  # (batch, num_atoms, hidden_dim)
        ...
        embeddings = [atom_emb, residue_emb, aa_pos_emb, seq_len_emb]

        # Optionally add atom position embedding
        if self.use_atom_pos_embed:
            atom_pos = torch.arange(num_atoms, device=atom_type.device).unsqueeze(0).expand(batch_size, -1)
            atom_pos_emb = self.atom_pos_embed(atom_pos)  # (batch, num_atoms, hidden_dim)
            embeddings.append(atom_pos_emb)
