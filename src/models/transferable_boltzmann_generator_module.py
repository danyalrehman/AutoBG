...
class TransferableBoltzmannGeneratorLitModule(LightningModule):
    ...
    def generate_samples(
        self, batch_size: int, encodings: dict[str, torch.Tensor] | None = None, n_timesteps: int = None
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def evaluate(
        ...
        # Use test sample count if running test or if eval_on_test is enabled
        use_test_samples = prefix.startswith("test") or (
            hasattr(self.datamodule, "hparams") and self.datamodule.hparams.get("eval_on_test", False)
        )
        if use_test_samples:
