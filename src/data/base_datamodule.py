...
class BaseDataModule(LightningDataModule):
    def __init__(
        ...
        :param eval_on_test: If True, use test set for validation during training. Defaults to `False`.
        """
        super().__init__()
    ...
    def val_dataloader(self) -> DataLoader[Any]:
        ...
        If eval_on_test=True, returns the test dataloader instead.

        :return: The validation dataloader.
        """
        # Use test set for validation if eval_on_test is enabled
        dataset = self.data_test if self.hparams.eval_on_test else self.data_val
        return DataLoader(
            dataset=dataset,
            batch_size=self.batch_size_per_device,
            num_workers=self.hparams.num_workers,
    ...
    def metrics_and_plots(
        ...
        smc_data: SamplesData | None = None,
        tica_model: TicaModel | None = None,
        prefix: str = "",
    ) -> None:
        """
