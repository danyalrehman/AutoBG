...
def train(cfg: DictConfig) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    # Set WANDB_PROGRAM to show the actual command with overrides in wandb UI
    try:
        from hydra.core.hydra_config import HydraConfig

        hydra_cfg = HydraConfig.get()
        overrides = hydra_cfg.overrides.task
        command = "python src/train.py " + " ".join(overrides)
        os.environ["WANDB_PROGRAM"] = command
    except ValueError:
        pass  # HydraConfig not initialized
    ...
    # The `train` flag is the sole train-vs-evaluate toggle:
    #   train=True  -> fit the model (optionally resuming from ckpt_path)
    #   train=False -> sample from and evaluate the checkpoint at ckpt_path
    if cfg.get("train", True):
        ...
        log.info("Starting evaluation (train=False)!")
